...
* ❌ **Pretentious & Hollow Adjectives**: *vital, crucial, bespoke, holistic, pivotal, paramount, multifaceted, ground-breaking, robust, comprehensive, seamless, cutting-edge, state-of-the-art, inspired by, ultra-fast, blazing fast.*
...
- **No Overloaded Compound Noun Stacks**: Eliminate chains of 4 or more consecutive modifiers and nouns before a head noun (e.g. *"interactive CLI-style hardware telemetry terminal component"*). Noun stacking without functional verbs is a primary machine-generation pattern. Replace with a compact subject anchored by concrete payload tokens (e.g. *"Built an in-browser Linux terminal component streaming live CPU, RAM, and OS metrics with zero Cumulative Layout Shift (CLS)"*).
- **No Paired Verb Coupling & Template Basins**: Ban inflated action verb pairs (*"Architected and deployed"*, *"Designed and implemented"*). When simplifying, never collapse into generic resume tropes (e.g. *"Built and deployed an accessible web portfolio with..."*); preserve domain-anchored architecture terms (*"Architected a responsive single-page web application..."*).
- **No Stacked Gerund Trios (The False Rule-of-Three)**: Ban forcing three `-ing` or nominalized verbs into a prepositional phrase (e.g. *"for navigation, theme toggling, and instant section jumping"*). State the primary functional purpose cleanly using active clauses anchored to concrete browser/DOM APIs (e.g. *"with global keydown listeners to focus the interactive CLI terminal and navigate sections"*). Never fall into generic template filler (*"to toggle themes and navigate between sections"*).
...
- **No Redundant Adverbs or Corporate Delivery Clichés**: Ban synthetic corporate phrasing like *"software deliveries shipped"*, redundant temporal adverbs like *"consistently every day"*, and LinkedIn template headers like *"Continuous GitHub Activity"*. Use authentic developer vernacular: *"GitHub Contribution Streak: Pushed code and opened pull requests every calendar day since..."*.
