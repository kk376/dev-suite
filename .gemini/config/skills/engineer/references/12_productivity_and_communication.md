...
- **No Paired Verb Coupling**: Ban inflated action verb pairs like *"Architected and deployed"*, *"Designed and implemented"*, *"Engineered and orchestrated"*. Use a single direct verb (`Built`, `Wrote`, `Shipped`, or simple coordination `Built and deployed`).
- **No Stacked Gerund Trios (The False Rule-of-Three)**: Ban forcing three `-ing` or nominalized verbs into a prepositional phrase (e.g. *"for navigation, theme toggling, and instant section jumping"*). State the primary functional purpose cleanly using active infinitive clauses (e.g. *"to toggle themes and navigate between sections"*).
