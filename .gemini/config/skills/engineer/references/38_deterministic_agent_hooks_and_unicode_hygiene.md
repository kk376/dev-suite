...
| **HTML & Markdown** | Hidden AI tool provenance comments (`<!-- generated by... -->`, C2PA tags). | CommonMark fence-aware regex cleaner |

---

## 6. CommonMark AI Provenance Scrubbing (`provenance-scrub`)

AI generation engines frequently inject hidden provenance comments into Markdown and HTML documents. These comments must be cleaned before release:

1. **Target Comment Patterns**:
   - `<!-- generated by ... -->`
   - `<!-- ai-generated ... -->`
   - `<!-- c2pa content credential ... -->`
   - `% generated by ...` (LaTeX)
   - `<!-- Created with ... -->` (SVG tool tags)
2. **CommonMark Code Fence Invariant**:
   Never strip comments occurring inside fenced code blocks (` ``` ` or ` ~~~ `). A valid CommonMark parser must enforce:
   - A fence opener whose info string contains a backtick is not a valid fence opener.
   - A closing fence line must contain only backticks (or tildes) matching or exceeding the opener length, with optional trailing whitespace. Trailing words (` ``` not a close `) do not close a code fence.
   - Comments inside valid fences must remain untouched so technical documentation examples are never corrupted.

---

## 7. Cross-Platform UTF-8 Standard Output Stream Hardening (`utf8-stream-guard`)

On Windows operating systems, piped standard output and standard error default to legacy ANSI code pages (such as `cp1252` on Western European/US installations). When a CLI tool or test runner outputs non-Latin characters (Devanagari, Cyrillic, CJK, Polish, Turkish) or technical glyphs, the process crashes with `UnicodeEncodeError`.

### 1. Python Stream Reconfiguration Standard
Every Python CLI tool, analysis script, or automation entry point must reconfigure standard streams to UTF-8 at process initialization:

```python
import sys

def configure_utf8_streams() -> None:
    """Enforce UTF-8 on stdout and stderr regardless of platform code page."""
...
### 2. CI Cross-Platform Stream Verification
Simulate legacy Windows ANSI encoding during Linux/macOS CI runs by executing child processes with forced encoding constraints:
```bash
PYTHONIOENCODING=cp1252 python -m pytest tests/test_tools_utf8_output.py
```

---

## 8. The Empirical "Write Like a Human" Prompt Collapse

Benchmark evaluations reveal an important operational lesson regarding LLM text styling:

- **The Formulaic Prompt Collapse**: Prompting an LLM with zero-shot conversational instructions such as *"write like a human"*, *"make this sound natural"*, or *"humanize this text"* actually causes statistical AI detector scores to collapse (in benchmark runs, collapsing human-likeness from 0.44 down to 0.02).
- **The Root Cause**: Conversational humanizer prompting forces the LLM into predictable, formulaic transitional patterns (*"Look,"*, *"Here's the thing,"*, *"At the end of the day,"*). These clichés form distinctive statistical clusters that detectors identify with near-100% certainty.
- **The Engineering Discipline**: Real human voice is achieved not through zero-shot emotional prompts, but through:
  1. Structural syntactic variety (alternating short and long clauses).
  2. Concrete DOM, OS, or algorithm mechanism anchors rather than vague adjectives.
  3. Elimination of robotic filler words and participle tails.
  4. Active voice and direct engineer-to-engineer communication cadence.
