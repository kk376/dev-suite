...
| "Review this PR / Review git diff / Shared-code audit" | Two-Axis Code Review & Shared-Code Rubric | `code-review` / `shared-code` / `reviewFreshness` | [`07_two_axis_code_review.md`](./references/07_two_axis_code_review.md) |
...
| "Git merge conflict / Rebase conflict / Quoted SQL guard" | Primary Source Resolution & Destructive Interceptor | `resolving-merge-conflicts` / `sql-guard` | [`11_merge_conflicts_and_git_guardrails.md`](./references/11_merge_conflicts_and_git_guardrails.md) |
...
| "Upstream RFC / Open source PR / Maintainers / Inbound PR review / PR body" | Upstream RFC, Maintainer Protocol & PR Body | `upstream-rfc` / `open-source` / `maintainer-pr` / `pr-body` | [`30_open_source_upstream_and_maintainer_collaboration.md`](./references/30_open_source_upstream_and_maintainer_collaboration.md) |
...
| "Strip invisible Unicode / Clean text / Zero emoji / Provenance scrub / Windows UTF-8" | Agent Hooks & Text Hygiene | `clean-text` / `zero-emoji` / `provenance-scrub` / `utf8-guard` | [`38_deterministic_agent_hooks_and_unicode_hygiene.md`](./references/38_deterministic_agent_hooks_and_unicode_hygiene.md) |
...
- **Working Tree Review Binding (`reviewFreshness`)**: Stamp `wtree` git tree hash before review begins. If tree content changes during or after review, mark status `STALE` and block release until re-verified.
- **Shared-Code Extraction Rubric**: Enforce the 5-rule anti-speculative helper extraction gate: prove 2+ first-party callers, reuse before extracting, keep helpers small, require positive net code savings, and rank reliability over syntactic similarity.
...
- **Diagnostic Endpoint Lockdown**: Gating all default framework documentation and debug endpoints in production (FastAPI `/docs`, Swagger UI, GraphQL introspection, Spring Boot Actuator, Werkzeug, Django `DEBUG=True`).
- **Quoted SQL Client Interception**: Peel shell wrappers (`sudo`, `env -C`) and intercept quoted destructive SQL (`psql -c "DROP TABLE..."`) while distinguishing safe data string literals.
...
- **Visual PR Body Architecture**: Structure PR descriptions for rapid human review: lead with the smallest visual summary (pseudocode, call tree, component tree, or diff sketch), pair with strict Before/After evidence, and classify reversibility (One-Way Door vs Two-Way Door) and blast radius.
- **Maintainer Inbound PR Review Protocol**: When receiving pull requests as a repository maintainer, never merge directly without a formal review. Follow the 5-phase protocol: (1) static triage and CI inspection (`gh pr view`, `gh pr diff`, `gh pr checks`), (2) local checkout and test suite verification (`gh pr checkout <PR>`, local test execution), (3) formal approval review submission (`gh pr review <PR> --approve -b "LGTM: Verified locally on $(uname -s) $(uname -m)..."`), (4) clean squash and merge (`gh pr merge <PR> --squash --delete-branch`), and (5) post-merge version bump, attribution in `CHANGELOG.md`, and multi-platform packaging dispatch.
- **GitHub Review Event Physics**: Direct merges via web UI or CLI emit only commit or merge events. Emitting a formal `PullRequestReviewEvent` on the maintainer's public GitHub contribution graph requires submitting an explicit review via `gh pr review`.
...
- [`11_merge_conflicts_and_git_guardrails.md`](./references/11_merge_conflicts_and_git_guardrails.md): Hunk-by-hunk conflict resolution, Two-Tier Destructive Command Matrix (careful/guard), quoted destructive SQL client interceptor, and signed commits.
...
- [`29_security_vibe_check_and_defensive_engineering.md`](./references/29_security_vibe_check_and_defensive_engineering.md): Defensive zero-trust engineering, 17 vibe-check security invariants, framework diagnostic endpoint lockdown (FastAPI, Swagger, GraphQL), automated audit loop, and Chief Security Officer (CSO) evidence-based verification.
- [`30_open_source_upstream_and_maintainer_collaboration.md`](./references/30_open_source_upstream_and_maintainer_collaboration.md): Open source upstream RFCs, maintainer collaboration, prototype maturation, thread consolidation, monorepo guardrails, inbound PR review protocol, GitHub profile review events, visual PR body architecture (pseudocode, call tree, diff sketch), One-Way vs Two-Way Door classification, and release pipelines.
...
- [`38_deterministic_agent_hooks_and_unicode_hygiene.md`](./references/38_deterministic_agent_hooks_and_unicode_hygiene.md): Deterministic agent harness hooks, atomic sibling swaps, Layer A invisible Unicode/Trojan Source scrubbing, CommonMark AI provenance comment cleaning, Windows UTF-8 stream hardening, zero-LLM mathematical stylometry, Zero-Emoji Standard, and metadata sanitization.
